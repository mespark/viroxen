...
BEGIN
  ...
  IF NEW.email_confirmed_at IS NOT NULL THEN
    ...
    ELSIF lower(NEW.email) = 'admin@example.com' THEN
...
CREATE POLICY "Staff read own tasks" ON public.tasks FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.staff s WHERE s.id = tasks.staff_id AND s.user_id = auth.uid()));
CREATE POLICY "Staff update own tasks" ON public.tasks FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.staff s WHERE s.id = tasks.staff_id AND s.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.staff s WHERE s.id = tasks.staff_id AND s.user_id = auth.uid()));
